...
export default function code128(
  ...
  var g = new Graphics(ctx, width, height, fb, bg);
...
function stringToCode128(text) {
  ...
  function getBestStartSet(csa1_, csa2_) {
    //tries to figure out the best codeset
    //to start with to get the most compact code
    var vote = 0;
    vote += csa1_ == CODESET.A ? 1 : 0;
    vote += csa1_ == CODESET.B ? -1 : 0;
    vote += csa2_ == CODESET.A ? 1 : 0;
    vote += csa2_ == CODESET.B ? -1 : 0;
    //tie goes to B due to my own predudices
    return vote > 0 ? CODESET.A : CODESET.B;
  }

  function perhapsCodeC(bytes_, codeset) {
    for (let _i = 0; _i < bytes_.length; _i++) {
      var b = bytes[_i];
...
var Graphics = function (ctx, width, height, fg = "#000", bg = "#fff") {
